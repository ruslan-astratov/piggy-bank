...
function isEmptyInputs() {
  if (
    goalName.value.trim() !== "" &&
    sumRequired.value.trim() !== "" &&
    period.value.trim() !== "" &&
    period.value.length >= 6 &&
    initialSum.value.trim() !== "" &&
    percent.value.trim() !== ""
  ) {
...
function expandСollapse(event) {
  if (
    ...
    event.target
      .closest(".target-card")
      .querySelector(".target-card-hidden-block").style.display = "block";
    ...
    event.target
      .closest(".target-card")
      .querySelector(".target-card-hidden-block").style.display = "none";
...
function saveOurTargetInTargetsArray() {
  ...
  if (
    ...
    let newTargetsArrayWithoutDublicates = removeDuplicates(
      arrForOurSaveTargets
    );

    console.log(
      "Массив без дубликатов newTargetsArrayWithoutDublicates",
      newTargetsArrayWithoutDublicates
    );
